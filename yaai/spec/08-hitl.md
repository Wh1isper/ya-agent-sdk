# Human-in-the-Loop (HITL) Design for yaai

## Overview

This document describes how yaai handles Human-in-the-Loop (HITL) for tool approval workflows. The key insight is that **HITL should be treated as an inner loop within agent execution, not as a separate TUI state**.
...
Location priority:
1. Project: `.yaai/tools.toml` (overrides global)
2. Global: `~/.yaai/tools.toml`

...
The configuration system provides a layered approach to managing TUI settings, allowing users to customize behavior while maintaining sensible defaults. Configuration is stored at `~/.yaai/` following XDG Base Directory specification.
...
class ConfigManager:
    ...
    DEFAULT_CONFIG_DIR = Path.home() / ".yaai"
    PROJECT_CONFIG_DIR = ".yaai"
    ...
    def __init__(
        ...
        self._config: YaaiConfig | None = None
        self._loaded_sources: list[str] = []

    @property
    def config(self) -> YaaiConfig:
        if self._config is None:
            self._config = self.load()
        return self._config

    def load(self) -> YaaiConfig:
        """Load and merge configuration from all sources."""
        merged: dict[str, Any] = {}

        # Layer 1: Builtin defaults (from YaaiConfig defaults)
        ...
        # Validate and return
        self._config = YaaiConfig.model_validate(merged)
        return self._config
    ...
    def _load_env_overrides(self) -> dict[str, Any]:
        ...
        env_mappings = {
            "YAAI_MODEL": ("general", "model"),
            "YAAI_AUTO_MODE": ("general", "auto_mode"),
            "YAAI_CDP_URL": ("browser", "cdp_url"),
            "YAAI_SESSION_DIR": ("session", "session_dir"),
        }
    ...
    def _generate_default_config_toml(self) -> str:
        ...
        return '''# YAAI CLI Configuration
# Generated by yaai init
...
yaai config set general.model "openai:gpt-4o"

# Get a configuration value
yaai config get general.model

# Reset to defaults
yaai config reset

# Validate configuration
yaai config validate

# Copy builtin subagent to user config for customization
yaai subagent copy search_agent

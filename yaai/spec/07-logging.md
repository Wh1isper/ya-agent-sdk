...
2. **Verbose mode** (`-v` flag): Logs written to `yaai.log` in current directory
...
| Verbose (`-v`) | FileHandler (`yaai.log`) | Queue + FileHandler |
...
class LogEvent(AgentEvent):
    ...
    logger_name: str = ""       # e.g., "yaai.session"
...
```python
from yaai.logging import configure_logging

# Silent mode (default)
configure_logging(verbose=False)

# Verbose mode - logs to yaai.log
...
```python
import asyncio
from yaai.logging import configure_tui_logging
...
3. **Simple**: `tail -f yaai.log` for real-time monitoring
...
- `verbose=True`: FileHandler (`yaai.log`)
...
Get a logger under the `yaai` namespace.
...
Format:
```
2026-01-15 14:30:00,123 DEBUG [yaai.session] Starting agent run

...
from yaai import __version__  # pyright: ignore[reportAttributeAccessIssue]
from yaai.config import ConfigManager, YaaiConfig
from yaai.logging import LOG_FILE_NAME, configure_logging, get_logger
...
def run_setup_wizard(config_manager: ConfigManager) -> bool:
    ...
    """
    click.echo()
    click.echo(click.style("Welcome to YAAI CLI!", fg="cyan", bold=True))
    ...
    if not config_path.exists():
        template_path = resources.files("yaai.templates").joinpath("config.toml")
    ...
    if not mcp_path.exists():
        mcp_template = resources.files("yaai.templates").joinpath("mcp.json")
    ...
    # Copy builtin skills from yaai (only missing directories - never overwrite)
    ...
    click.echo()
    click.echo("Run 'yaai' again to start!")
    click.echo()

    return True


def load_env_from_config(config: YaaiConfig) -> None:
...
def _ensure_builtin_skills(config_dir: Path) -> list[str]:
    ...
    Copies builtin skills from yaai.skills to the config directory.
    ...
    builtin_skills = resources.files("yaai.skills")
...
@click.version_option(version=__version__, prog_name="yaai")
def cli(verbose: bool) -> None:
    """YAAI CLI - AI-powered coding assistant.
    ...
    logger.info("Starting yaai v%s", __version__)

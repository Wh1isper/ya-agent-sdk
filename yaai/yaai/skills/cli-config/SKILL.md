---
name: cli-config
description: Guide for configuring YAAI CLI. Use this skill when users want to configure models, tools, subagents, custom commands, or other CLI settings. Covers both global and project-level configuration.
---
...
Project-level tool permissions in `.yaai/tools.toml`:
...
Create markdown files in `~/.yaai/subagents/`:
...
1. Built-in: `yaai/skills/` (package bundled)
2. Global: `~/.yaai/skills/`
3. Project: `.yaai/skills/`

## Environment Variables

TUI settings can be overridden via `YAAI_*` environment variables:

- `YAAI_CODE_THEME`
- `YAAI_SHOW_TOKEN_USAGE`
- `YAAI_CDP_URL`
- `YAAI_SESSION_DIR`

## Quick Setup

Run `yaai setup` to initialize global configuration with defaults.

"""Configuration management for yaai.

Configuration files are loaded with project-level priority (no merging):

1. **config.toml** (model + TUI settings):
   - Global: ~/.yaai/config.toml
   - Project: .yaai/config.toml (overrides global entirely)
   - Contains: model, model_settings, display, steering, session, browser, subagents, env

2. **tools.toml** (tool permissions):
   - Global: ~/.yaai/tools.toml
   - Project: .yaai/tools.toml (overrides global entirely)
   - Contains: need_approval list

3. **mcp.json** (MCP server configurations):
   - Global: ~/.yaai/mcp.json
   - Project: .yaai/mcp.json (overrides global entirely)

4. **Environment variables** (YAAI_*):
...
class SubagentsConfig(BaseModel):
    ...
    Subagents are loaded from ~/.yaai/subagents/
    which is initialized by `yaai setup`.
    """
...
class ConfigManager:
    ...
    def get_auto_save_dir(self) -> Path:
        ...
        Returns:
            ...
            Format: ~/.yaai/message_history/{project_hash}/
        """
...
def _load_template(name: str) -> str:
    ...
    template_files = resources.files("yaai").joinpath("templates")

...
from yaai.app.state import TUIMode
from yaai.browser import BrowserManager
from yaai.config import ConfigManager, YaaiConfig
from yaai.display import EventRenderer, RichRenderer, ToolMessage
from yaai.environment import TUIEnvironment
from yaai.events import ContextUpdateEvent
from yaai.hooks import emit_context_update
from yaai.logging import configure_tui_logging, get_logger
from yaai.runtime import create_tui_runtime
from yaai.session import TUIContext
from yaai.usage import SessionUsage
...
class TUIApp:
    ...
    def _dump_history(self, folder_path: str | None) -> None:
        ...
        Args:
            folder_path: Target folder path. Defaults to ".yaai-session".
        """
        if not self._message_history:
            self._append_system_output("No conversation history to dump")
            return

        dump_dir = Path(folder_path or ".yaai-session").expanduser().resolve()
        try:
            # Create folder
    ...
    def _auto_save_history(self) -> None:
        ...
        ~/.yaai/message_history/{project_hash}/
    ...
    async def run(self) -> None:
        ...
        # Welcome message
        title = Text("YAAI CLI", style="bold magenta")

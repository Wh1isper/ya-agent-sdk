...
| Home          | Command-first start                  | Home                            | UI wired                  | UI local                     | Claw backend ready           | Move Space/Profile defaults into durable Desktop Core settings.             | P1       |
| Home          | Recent chats                         | Home                            | UI wired                  | N/A                          | Claw backend ready           | Add richer sorting and pinned chats.                                        | P0       |
| Home          | Active runs                          | Home, right context             | UI wired                  | N/A                          | Claw backend ready           | Add richer active-run progress and native notifications.                    | P1       |
| Home          | Current space summary                | Home                            | UI wired                  | UI local                     | Workspace binding ready      | Promote local Space registry to Desktop Core storage.                       | P1       |
| Chats         | Chat list                            | Chats                           | UI wired                  | N/A                          | Claw backend ready           | Add search, filtering, and stable chat titles.                              | P0       |
| Chats         | Chat detail                          | Chats                           | UI wired                  | N/A                          | Claw backend ready           | Add selected run drilldown and artifact expansion.                          | P0       |
...
| Chats         | Streaming run output                 | Chats, Home                     | UI wired                  | N/A                          | Claw backend ready           | Add markdown rendering and tool-call side panels.                           | P1       |
| Chats         | Continue chat                        | Chats                           | UI wired                  | N/A                          | Claw backend ready           | Add attachment/context part selection.                                      | P1       |
...
| Chats         | Cancel active run                    | Chats, Inbox                    | UI wired                  | N/A                          | Claw backend ready           | Add confirmation and cancellation reason display.                           | P1       |
| Board         | Kanban grouped by run/session status | Board                           | UI wired                  | N/A                          | Claw backend ready           | Add filters and route-to-chat navigation.                                   | P1       |
...
| Spaces        | Local folder registry                | Spaces                          | UI local                  | API contract needed          | N/A                          | Promote folders, trust, defaults, and mount presets to Desktop Core.        | P1       |
| Spaces        | Workspace trust                      | Spaces, right context           | UI local                  | Product model needed         | Workspace validation ready   | Define trust levels and execution restrictions.                             | P1       |
| Spaces        | Session workspace binding            | Spaces, Chats                   | UI wired                  | UI local                     | Claw backend ready           | Add multi-mount editing and trust validation.                               | P1       |
...
| Inbox         | Pending approvals                    | Inbox, right context            | UI wired                  | Native notifications pending | API contract documented      | Add approve/deny response cards.                                            | P1       |
| Inbox         | Failed background work               | Inbox, Board                    | UI wired                  | N/A                          | Claw backend ready           | Add one-click recovery/rerun.                                               | P1       |
...
| Right context | Live connection/session/run context  | Right panel                     | UI wired                  | N/A                          | Claw backend ready           | Add memory, trace, and artifact summaries.                                  | P1       |
| Notifications | Global SSE events                    | App shell, Inbox, tray          | UI wired                  | N/A                          | Claw backend ready           | Add visible notification center and native notification mapping.            | P1       |
...
| Profiles      | Runtime profiles                     | Settings, new chat              | UI wired                  | N/A                          | Claw backend ready           | Add profile detail and default profile settings.                            | P1       |
...
2. Chats can continue an existing session, show streaming output, cancel active runs, and display run state transitions. Done.
3. Board renders from the shared Desktop chat read model. Done.
4. Spaces stores local folders, trust, default workspace, and mount presets in local Desktop state. Done.
5. Inbox surfaces failed runs and pending approval items. Done.
6. Global SSE updates React Query caches and right context state. Done.
7. Profiles load from Claw and can be selected for new chats. Continuations use the session profile. Done.

...
The model-facing prompt and visible transcript contain one catalog-grounded `Use this skill: <name>` line per selected skill, followed by the remaining task as `User instructions: <task>` when present. They do not inject skill bodies. Prompt history retains the original slash input for editing and reuse. The agent resolves each selected name through the available-skill catalog, inspects its `SKILL.md`, and applies the SDK skill activation policy.

...
class TestSessionUsage:
    ...
    def test_uses_sdk_cost_estimate(self) -> None:
        """SDK-provided estimates should replace local model price tables."""
        session = SessionUsage()
        session.add(
            "main",
            "xai:grok-4.5",
            ...
            cost_estimate=CostEstimate(
                input_amount=Decimal("1.70"),
                output_amount=Decimal("0.60"),
                total_amount=Decimal("2.30"),
                priced_requests=1,
            ),
        )
        ...
        assert "Estimated Cost: $2.30 (1/1 requests priced)" in summary
        assert session.estimated_total_cost is not None
        assert session.estimated_total_cost.total_amount == Decimal("2.30")

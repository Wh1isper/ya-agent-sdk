...
_RUN_CUSTOM_EVENT_PREFIX = "yaacli"
...
class DisplayEventAdapter:
    ...
    def build_system_event(self, event_name: str, payload: JsonValue) -> dict[str, Any]:
        return _dump_agui_event(CustomEvent(name=f"{_RUN_CUSTOM_EVENT_PREFIX}.{event_name}", value=payload))
    ...
    def _adapt_part_start(self, stream_event: StreamEvent, cursor: AgentCursor) -> list[dict[str, Any]]:
        ...
        if isinstance(part, TextPart):
            ...
            if part.content:
                events.append(
                    _dump_agui_event(
                        TextMessageChunkEvent(
                            message_id=message_id, role="assistant", name=stream_event.agent_name, delta=part.content
                        )
                    )
                )
        ...
        if isinstance(part, ToolCallPart):
            ...
            cursor.parts[event.index] = PartCursor(
                kind="tool_call", part_id=tool_call_id, tool_call_name=part.tool_name
            )
            ...
            if chunk_delta is not None or part.tool_name:
                events.append(
                    _dump_agui_event(
                        ToolCallChunkEvent(tool_call_id=tool_call_id, tool_call_name=part.tool_name, delta=chunk_delta)
                    )
                )
                cursor.parts[event.index].emitted_chunk = True
            return events
    ...
    def _ensure_text_cursor(self, agent_id: str, cursor: AgentCursor, index: int) -> PartCursor:
        ...
        part_cursor = PartCursor(
            kind="text", part_id=self._part_id(agent_id, cursor.loop_index, index, "text"), role="assistant"
        )
    ...
    def _ensure_reasoning_cursor(self, agent_id: str, cursor: AgentCursor, index: int) -> PartCursor:
        ...
        if existing is not None:
            return existing
        part_cursor = PartCursor(
            kind="reasoning", part_id=self._part_id(agent_id, cursor.loop_index, index, "reasoning"), role="reasoning"
        )
        cursor.parts[index] = part_cursor
        return part_cursor

    def _ensure_tool_call_cursor(
        self, agent_id: str, cursor: AgentCursor, index: int, tool_call_id: str | None
    ) -> PartCursor:
        existing = cursor.parts.get(index)
        if existing is not None:
            if tool_call_id:
            ...
            return existing
        part_cursor = PartCursor(
            kind="tool_call", part_id=tool_call_id or self._part_id(agent_id, cursor.loop_index, index, "tool_call")
        )
        cursor.parts[index] = part_cursor
        return part_cursor
...
def validate_display_events(raw_payload: object) -> list[dict[str, Any]]:
    if not isinstance(raw_payload, list):
        raise TypeError("display_messages payload must be a top-level JSON array of event objects")
    events = [event for event in raw_payload if isinstance(event, dict)]
    if len(events) != len(raw_payload):
        raise TypeError("display_messages payload must contain only event objects")
    return events
...
def _event_field(event: dict[str, Any], camel_name: str, snake_name: str) -> object:

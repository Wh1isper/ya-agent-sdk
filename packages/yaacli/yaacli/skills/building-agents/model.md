...
`AgentContext` initializes `provider_session_id` once from its initial `run_id` unless the host supplies a durable identity. The value survives `prepare_new_run()` while native run IDs remain fresh unless `provider_thread_id` explicitly pins the thread. Each subagent uses its root child execution ID as a distinct provider session that remains stable across retry and linked continuation. For transports that receive SDK context headers, the SDK resolves provider routing from the active `RunContext` on every root and child model request. It copies the settings resolved so far and replaces all provider session/thread header aliases with `AgentContext.get_model_extra_headers()`, so one reusable runtime keeps stable default routing and can also serve explicitly rebound durable sessions. When the active `ModelConfig` explicitly declares `ModelFeature.openai_prompt_cache_key`, the SDK also sets `openai_prompt_cache_key` to exactly the current `x-session-id`. The context session is authoritative: conflicting request-level provider headers and `extra_body["prompt_cache_key"]` overrides are normalized or removed on the copied settings. Existing caller-provided cache keys are replaced so the two routing values cannot diverge; neither the top-level settings nor nested mappings supplied by the caller are mutated.

The `gpt5_270k`, `gpt5_350k`, and `gpt5_1m` model-config presets and their GPT-6 counterparts (`gpt6_270k`, `gpt6_350k`, and `gpt6_1m`) declare this capability. The GPT-6 presets duplicate the corresponding GPT-5 configurations, including the 270,000 / 350,000 / 922,000 token context budgets. The `gpt6` alias selects `gpt6_270k`; existing `gpt5` and `openai` defaults are unchanged. GPT-6 reasoning settings use the shared `openai_responses_*` presets. Other presets do not declare this capability. Custom model configurations can opt in only after confirming provider support:
...
Declaring the capability alone does not invent a provider session header. The SDK currently supplies `x-session-id` for `oauth@codex:`, direct `openai-responses-rs:` / `openai-responses-ws:`, and gateway-backed `openai-responses:`, `openai-responses-rs:`, and `openai-responses-ws:` transports. Gateway HTTP requests and WebSocket HTTP fallback use the same context headers as the WebSocket handshake. Other gateway upstream providers, including `openai-chat`, are not opted into automatic context headers.

...
For matching GPT-6 configuration names, use `gpt6_270k`, `gpt6_350k`, or `gpt6_1m`. These duplicate the corresponding GPT-5 presets, including their 270,000 / 350,000 / 922,000 token context budgets and capabilities; they do not infer new model limits. The `gpt6` alias selects `gpt6_270k`, while `gpt5` and `openai` keep their existing defaults. Reasoning settings continue to use the shared `openai_responses_*` presets.

The GPT-5 and GPT-6 model-config presets also declare `ModelFeature.openai_prompt_cache_key`. `AgentContext` initializes `provider_session_id` once from its initial `run_id` unless the host supplies an explicit durable identity, and preserves it while native run IDs remain fresh. Each subagent uses its root child execution identity as a separate provider session, so it never shares the main agent's `x-session-id` and linked continuation keeps the same child value. For transports that receive the SDK's provider session headers, every root and child model request copies the settings resolved so far, replaces all provider session/thread header aliases from the active context, and binds `openai_prompt_cache_key` to the exact current `x-session-id` value. This per-request binding lets one reusable runtime keep one stable default provider session or serve explicitly rebound durable sessions without leaking the identity captured at `create_agent()` time. It includes OAuth Codex, direct Responses WebSocket aliases, and gateway-backed OpenAI Responses over HTTP or WebSocket. Gateway HTTP requests and WebSocket fallback receive the same headers as the handshake. Conflicting request-level header or body overrides are normalized on the copy; caller-owned mappings are not mutated. Models without the explicit cache-key capability still receive current provider headers but do not receive an automatic prompt cache key.
...
ordering can fail before durable admission. Static validation and runtime entry reject
duplicate non-null capability IDs before Pydantic AI's native capability merging, so
declarative and host policies cannot silently overwrite or merge one another. Native
spec factories run once per construction and the caller's spec remains unchanged.
Runtime entry still validates dynamic Environment, context, and resource contributions.

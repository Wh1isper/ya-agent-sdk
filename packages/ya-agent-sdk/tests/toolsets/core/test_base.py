...
def test_toolset_with_subagents_history_processors_deprecated(agent_context: AgentContext) -> None:
    """Should keep public history_processors compatibility for individual subagents."""
    from pydantic_ai.messages import ModelMessage
    from ya_agent_sdk.subagents import SubagentConfig

    def processor(ctx: RunContext[AgentContext], messages: list[ModelMessage]) -> list[ModelMessage]:
        return messages
    ...
    with pytest.warns(DeprecationWarning, match="history_processors"):
        result = toolset.with_subagents([config], history_processors=[processor])

    assert "test_subagent" in result.tool_names


def test_toolset_with_unified_subagents_history_processors_deprecated(agent_context: AgentContext) -> None:
    """Should keep public history_processors compatibility for unified subagents."""
    from pydantic_ai.messages import ModelMessage
    from ya_agent_sdk.subagents import SubagentConfig
    ...
    with pytest.warns(DeprecationWarning, match="history_processors"):
        result = toolset.with_subagents([config], history_processors=[processor], unified=True)

    assert "delegate" in result.tool_names

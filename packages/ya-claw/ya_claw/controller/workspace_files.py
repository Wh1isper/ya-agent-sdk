...
def _validated_virtual_path(path: str | None, *, default: str | None = None) -> str:
    ...
    if "\x00" in raw_path or "\\" in raw_path or not raw_path.startswith("/"):
...
def _require_secure_fd_operations(virtual_path: str, *, require_scandir: bool = False) -> None:
    if (
        ...
        or (require_scandir and not _SCANDIR_SUPPORTS_FD)
    ):
        # Never fall back to path-based traversal or silently follow links on platforms
        # without the descriptor primitives needed to enforce the mount boundary.
        raise HTTPException(
            status_code=403,
...
def _entry_kind(mode: int) -> WorkspaceFileKind:

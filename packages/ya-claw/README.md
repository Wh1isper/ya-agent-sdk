...
- `DockerWorkspaceProvider` uses Docker mounts through `SandboxEnvironment`; file operations map the service-visible workspace path to `/workspace`, and Docker shell uses `/workspace`. Managed temporary storage lives at a hidden `.ya-agent/tmp/<id>` path below that shared workspace mount, including for reused containers; no per-run bind mount is required.

...
- GitHub CLI (`gh`)
- `lark-cli`
- bundled workspace skills copied into mounted workspace `.agents/skills/` directories at container start
...
Workspace environments receive built-in `GH_TOKEN` from `YA_CLAW_BRIDGE_GITHUB_TOKEN` and built-in `LARK_APP_ID` / `LARK_APP_SECRET` aliases from process environment values or the configured Lark bridge app settings. `YA_CLAW_WORKSPACE_ENV_VARS` forwards additional comma-separated process environment variable names into workspace environments; values are read from the YA Claw service process environment and passed to local shell execution or Docker container creation. Forwarded values may override Lark aliases, but the GitHub bridge token is merged last so a forwarded host `GH_TOKEN` cannot change the account or authority used by bridge sessions. `YA_CLAW_WORKSPACE_PROVIDER_DOCKER_EXTRA_MOUNTS` mounts additional host directories into Docker workspace containers using comma-separated `host_path:container_path[:mode]` entries with `rw` and `ro` modes. Extra mounts are provider support mounts: they are appended to Docker environment mounts, included in sandbox fingerprints, and excluded from workspace default cwd, guidance root, memory root, and Desktop Space association.

...
- `[general]` and `[model_profiles.*]` support static `instructions` for the active main-agent model profile; YAACLI registers them through a dynamic Pydantic AI system-prompt callback solely so `/model` switches replace the instruction on later history-backed requests, and the SDK base system-prompt filter preserves dynamic prompt parts for reevaluation
